...
vignette: >
  %\VignetteIndexEntry{Function gallery}
...
* `absorbLayer`: "Absorbs" all or part of a layer into the surrounding colors, optionally according to a size or location condition. (Not run on CRAN.)
...
```{r, fig.width = 6}
rc_edit <- editLayer(recluster_fit,
                      layer_idx = 3,
                      operation = "fill",
                      px_size = 2)
```
...
* `classify_recolorize`: Converts a `recolorize` object to a `classify` object in the [pavo](https://rdrr.io/cran/pavo/) package for linking with spectral data.
